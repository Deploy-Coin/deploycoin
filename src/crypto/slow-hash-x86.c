...
/* This file contains the x86 version of the Cryptonight-Dev slow-hash routines */
...
/*
 * An SSE-optimized implementation of the second half of Cryptonight-Dev step 3.
...
STATIC INLINE int force_software_aes(void)
{
    ...
    const char *env = getenv("DeployCOIN_USE_SOFTWARE_AES");
    if (!env)
    {
        use = 0;
    }
 ...
 * the large 2MB buffer.  Note that Cryptonight-Dev does not use a completely
 ...
 *
 * To fill its 2MB scratch buffer, Cryptonight-Dev uses a nonstandard implementation
 ...
 * important speed optimizations needed to make Cryptonight-Dev faster.
 *
...
/**
 * @brief the hash function implementing Cryptonight-Dev, used for the Monero proof-of-work
 *
 * Computes the hash of <data> (which consists of <length> bytes), returning the
 * hash in <hash>.  The Cryptonight-Dev hash operates by first using Keccak 1600,
 ...
 * https://www.cs.cmu.edu/~dga/crypto/xmr/Cryptonight-Dev.png
 *
    ...
    uint32_t iterations)
{
    ...
    /* Cryptonight-Dev Step 1:  Use Keccak1600 to initialize the 'state' (and 'text') buffers from the data. */
    if (prehashed)
    {
    ...
    /* Cryptonight-Dev Step 2:  Iteratively encrypt the results from Keccak to fill
    ...
    /* Cryptonight-Dev Step 3:  Bounce randomly 1,048,576 times (1<<20) through the mixing buffer,
    ...
    /* Cryptonight-Dev Step 4:  Sequentially pass through the mixing buffer and use 10 rounds
    ...
    /* Cryptonight-Dev Step 5:  Apply Keccak to the state again, and then
