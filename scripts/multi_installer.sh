...
# Deploycoin Multi-installer
# a one line clone-and-compile for Deploycoin:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/Deploycoin/Deploycoin/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/Deploycoin ]; then
            read -r -p "${1:-Deploycoin directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old Deploycoin directory" && echo
                    rm -rf "$PWD"/Deploycoin
                    ;;
                *)
                    _fail "Deploycoin directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir Deploycoin
        git clone -b master -q https://github.com/Deploycoin/Deploycoin Deploycoin   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd Deploycoin
    fi
}

_build_Deploycoin() {
    _note "Building Deploycoin from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/Deploycoin')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/Deploycoin')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Deploycoin Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'Deploycoin/build/src/' for the executible binaries. See 'https://github.com/Deploycoin/Deploycoin' for more project support. Cowabunga!"
