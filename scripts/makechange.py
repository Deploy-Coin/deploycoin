...
Open the wallets with Deploy-serivce like so:

./Deploy-service -w walletA.wallet -p yourpass --rpc-password test --bind-port 8070
./Deploy-service -w walletB.wallet -p yourpass --rpc-password test --bind-port 8071
...
moveDecimal = 100  # DE has 2 decimals so 100 is the divide/multiply factor
minAmount = 100 * moveDecimal  # min number for amount to xfer
maxAmount = 5000 * moveDecimal  # max number for amount to xfer
anonymity = 3
fee = 10 # atomic units, DE would be 0.10 as the tx network fee
